...
- [x] Diagnostics/style check: `dotnet format <solution> --verify-no-changes --severity info` (mirrors
      the AGENTS.md "IDE/style diagnostics pass"). Fails the job on any drift at `info` severity or
      above — i.e. everything, since `info` is the lowest severity `dotnet format` recognizes. **This
      was originally implemented as a non-failing annotation, then reversed (see Decision Log).**
