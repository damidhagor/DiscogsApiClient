...
- [ ] `NUGET_USER` — the nuget.org account **username** (not email) used for both nuget.org and int.nugettest.org trusted-publishing logins, repository secret **(pending: must be added in repository settings by a maintainer; not something an agent/CI change can configure)**
- [ ] Trusted Publishing policy registered on **nuget.org** (Repository Owner = `damidhagor`, Repository = `DiscogsApiClient`, Workflow File = `publish-nuget.yml`, no Environment) **(pending: nuget.org account action, not something an agent/CI change can configure)**
- [ ] Trusted Publishing policy registered on **int.nugettest.org**, same values as above **(pending, same reason)**
...
- [ ] At least one successful end-to-end test-server publish validated — pending Trusted Publishing policy registration (7.4) and an actual `workflow_dispatch` run against GitHub Actions
- [ ] `NUGET_USER` secret and both Trusted Publishing policies configured — pending maintainer action
