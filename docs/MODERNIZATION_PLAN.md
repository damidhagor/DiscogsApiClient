...
- [x] Restore and build (all target frameworks) as a gate — `ci-library.yml`
      ...
      .NET 10 only — demos don't multi-target). **Deviation:** any Warning-severity analyzer/style
      diagnostic (from `-p:EnforceCodeStyleInBuild=true`) also fails the build step via `-warnaserror`,
      not just compiler errors — this reversed an earlier non-failing-annotation design (see Decision
      Log).
...
- [x] Diagnostics/style check: `.github/scripts/format-check.sh` runs `dotnet format <solution>
      --verify-no-changes --severity info --exclude-diagnostics IDE0005 IDE0060` (mirrors the AGENTS.md
      "IDE/style diagnostics pass", minus the two excluded rules). Fails the job on any drift at `info`
      severity or above — i.e. everything else, since `info` is the lowest severity `dotnet format`
      recognizes. **This was originally implemented as a non-failing annotation, then reversed (see
      Decision Log).** `IDE0005`/`IDE0060` are excluded because `dotnet format`'s `MSBuildWorkspace` never
      resolves the source generator, causing systematic false positives; `IDE0060` is instead enforced by
      the real `dotnet build` above (generator-aware), `IDE0005` is left IDE-only (see Decision Log and
      `docs/CI_CD.md`). The script also groups `dotnet format`'s plain-text output into a deduplicated
      job-summary table (Rule | Severity | Message | Locations).
...
- [x] Trigger: `workflow_dispatch` only (no tag-push or branch-push trigger) — every release is a deliberate, manual action
- [x] Manual input: boolean switch (e.g. `publish_to_test_server`, default `false`) — when `true`, the publish step targets `int.nugettest.org` instead of `nuget.org`
- [x] Build (Release configuration) as a gate before packing — **deviation from the original plan:** no test suite run and no restore/multi-SDK setup. Quality (build+tests+format+vulnerabilities) is already fully enforced per-PR by `ci-library.yml`/`dependency-check.yml` before code ever reaches `main`, so re-running the test suite here would just duplicate a check that already passed; a single `10.0.x` SDK compiles all three target frameworks fine without needing matching installed runtimes, since nothing here executes test binaries. Validated locally: `dotnet build src/DiscogsApiClient/DiscogsApiClient.csproj -c Release` succeeds cleanly and produces net8.0/net9.0/net10.0 outputs.
- [x] `dotnet pack` only the `DiscogsApiClient` library project (not test/demo projects) in Release configuration, producing `.nupkg` + `.snupkg` (symbol package) — `IncludeSymbols`/`SymbolPackageFormat=snupkg` passed as one-off pack properties (not added to the `.csproj`, to avoid a plain local `dotnet pack` starting to emit a `.snupkg` as a side effect); validated locally, both files produced. **Note:** `dotnet pack` alone does *not* build first even though `GeneratePackageOnBuild=True` is set (confirmed locally: fails with `NU5026`, "file to be packed was not found on disk"), so the preceding explicit build step is required, not optional.
- [x] Upload the produced package(s) as a workflow artifact regardless of publish outcome (so a failed/aborted publish still leaves the built package retrievable) — upload step runs immediately after packing, before the push step
- [x] Push the package via `dotnet nuget push`:
  - [x] Default (`publish_to_test_server = false`): push to `https://api.nuget.org/v3/index.json`
  - [x] Test mode (`publish_to_test_server = true`): push to `https://apiint.nugettest.org/v3/index.json`
  - **Deviation from the original plan:** authenticates via **NuGet Trusted Publishing** (OIDC, `NuGet/login@v1`) instead of long-lived `NUGET_API_KEY`/`NUGET_TEST_API_KEY` secrets — see the "Authentication" sub-item below and §7.4.
  - Confirmed via Microsoft Learn docs that `dotnet nuget push` pushes the matching `.snupkg` alongside each `.nupkg` automatically (only `--no-symbols` suppresses it) — no separate symbol-push command needed.
- [x] **Authentication: NuGet Trusted Publishing (OIDC), not API keys** — **deviation from the original plan**, made after the user flagged that nuget.org discourages API keys in favor of [Trusted Publishing](https://learn.microsoft.com/en-us/nuget/nuget-org/trusted-publishing). Since neither `NUGET_API_KEY` secret had actually been configured yet, switched straight to OIDC instead of migrating later: `permissions: id-token: write` added at job level; two mutually-exclusive `NuGet/login@v1` steps (`login_prod`/`login_test`, gated by the `publish_to_test_server` input) exchange the GitHub OIDC token for a short-lived (1 hour, single-use) NuGet API key — production uses the action's defaults, the test-server step overrides `token-service-url`/`audience` to `int.nugettest.org` (confirmed via `NuGet/login`'s README that these are supported inputs, and via community reports that int.nugettest.org supports trusted publishing the same way nuget.org does). Both use one `secrets.NUGET_USER` (nuget.org **username**, not email) per user's confirmation that the same username will be used on both targets. The push step's `--api-key` reads whichever login step actually ran: `steps.login_prod.outputs.NUGET_API_KEY || steps.login_test.outputs.NUGET_API_KEY`.
- [x] Workflow file: `.github/workflows/publish-nuget.yml`

### 7.3 Versioning
- [x] Package version is read as-is from the existing `<PackageVersion>`/`<AssemblyVersion>`/`<FileVersion>` properties already hand-maintained in `DiscogsApiClient.csproj` — no automated version-bump tooling introduced in this phase. Resolved via `dotnet msbuild -getProperty:PackageVersion` (MSBuild 17.8+/.NET 8 SDK+ feature; validated locally, correctly printed `5.0.0`)
- [x] Workflow surfaces the resolved version in its run summary/logs so the person triggering it can confirm what will be published before approving — written to `$GITHUB_STEP_SUMMARY` alongside the resolved push target (test server vs. production)

### 7.4 Secrets
- [x] ~~`NUGET_API_KEY` — nuget.org API key, repository secret~~ — **superseded:** no longer needed at all. Replaced by NuGet Trusted Publishing (OIDC); see the Authentication deviation note in §7.2.
- [x] ~~`NUGET_TEST_API_KEY` — separate API key for `int.nugettest.org`~~ — **superseded**, same reason.
- [x] `NUGET_USER` — the nuget.org account **username** (`DamIDhagor`, not email) used for both nuget.org and int.nugettest.org trusted-publishing logins, repository secret — configured by the maintainer
- [x] Trusted Publishing policy registered on **int.nugettest.org** (Policy Name = `DiscogsApiClient-github`, Repository Owner = `damidhagor`, Repository = `DiscogsApiClient`, Workflow File = `publish-nuget.yml`, no Environment) — configured by the maintainer and validated by an actual `workflow_dispatch` run (see Acceptance Criteria)
- [x] No GitHub Environment protection/required-reviewer gate — single-maintainer project, the manual `workflow_dispatch` + explicit boolean switch is a sufficient safeguard; user explicitly opted to skip adding a GitHub Environment for now
- **Moved to Phase 8.3:** the **nuget.org (production)** Trusted Publishing policy is deliberately not registered yet — it only matters at the actual v5.0.0 release, so its setup is tracked there instead of blocking Phase 7 completion (see §8.3 for the exact policy field values)

### 7.5 Documentation
- [x] Document both workflows (what the PR-validation workflow enforces; how to trigger the release workflow, what the test-server switch does, where to find the resulting package artifact) in `docs/` or the README — see `docs/CI_CD.md`

### Acceptance Criteria - Phase 7
- [x] PR-validation workflow file exists and runs build + tests + format check + vulnerability check on every PR
- [x] Publish workflow file exists (e.g. `.github/workflows/publish-nuget.yml`)
- [x] Publish workflow builds and packs the library before any publish step runs (no test re-run — see 7.2 deviation note)
- [x] Publish workflow can be manually triggered with the test-server switch, defaulting to nuget.org
- [x] At least one successful end-to-end test-server publish validated — `workflow_dispatch` run `32525525184` on `phase7-2-nuget-publish-workflow` (dispatched against a temporary workflow-only PR #21 merged to `main`, since `workflow_dispatch` requires the workflow file to exist on the default branch to be dispatchable at all) completed successfully: build → pack → OIDC login (`int.nugettest.org`) → push both `DiscogsApiClient.5.0.0.nupkg` and `.snupkg` (confirmed via job logs: two separate "Your package was pushed." messages for `/api/v2/package` and `/api/v2/symbolpackage`). Package indexed and confirmed visible via `https://apiint.nugettest.org/v3-flatcontainer/discogsapiclient/index.json` shortly after
- [x] `NUGET_USER` secret and the int.nugettest.org Trusted Publishing policy configured and validated; the nuget.org (production) policy is intentionally deferred to Phase 8.3, right before the real release
- [x] Both workflows documented for future use
...
### 8.3 Release
- [ ] Register the nuget.org Trusted Publishing policy (moved here from Phase 7.4 — production publishing
      is only actually needed at this step, and the policy takes effect immediately once created, so
      creating it right before the real release avoids a long-dormant production credential path). Go to
      [nuget.org/account/trustedpublishing](https://www.nuget.org/account/trustedpublishing) → **Add
      policy** with:
      - **Policy Name:** `DiscogsApiClient-github` (matches the name already used for the int.nugettest.org
        policy, purely for consistency — this field is a free-text label with no bearing on matching)
      - **Package Owner:** the nuget.org account/org that owns the `DiscogsApiClient` package
      - **Package ID:** `DiscogsApiClient`
      - **Repository Owner:** `damidhagor`
      - **Repository:** `DiscogsApiClient`
      ...
      - If the repo is private at that point: the policy is only "temporarily active" for 7 days until the
        first successful publish — publish within that window.
      - The `NUGET_USER` repository secret is already set to `DamIDhagor` (nuget.org username, not email) —
        reuse the same value, no change needed here.
...
| 2026-08-21 | `annotate-diagnostics.sh` now also writes a markdown table of every warning to the job summary | The same clean-state PR run showed that `::warning::` annotations pointing at files/lines outside the PR's diff (e.g. pre-existing analyzer warnings in untouched `DiscogsApiClient.cs`) never surface on the PR conversation page or its checks/merge widget — GitHub only attaches them inline when the line is part of that PR's diff, otherwise they're only visible on the workflow run's own Annotations page | Reviewers can now see every build/format warning from the job summary (top of each check's run page) without needing to know to look for a separate Annotations panel; `docs/CI_CD.md`'s Guiding Principle section documents this GitHub visibility limitation |
| 2026-08-21 | Discovered the job-summary table above was silently broken (always reported "No warnings found" despite real warnings/annotations existing), root-caused, then the whole non-failing-annotation design was reversed to a strict fail-on-any-diagnostic policy | Root cause of the summary bug: `dotnet build`/`dotnet format` detect `GITHUB_ACTIONS=true` and emit their own native `##[warning]`/`##[error]` annotations directly — but to **stderr**, while `... \| tee build.log` only captured stdout, so the log files `annotate-diagnostics.sh` parsed never contained them (fixed by adding `2>&1`, confirmed locally: script then correctly parsed and re-emitted all 20 real CA1822/CA1823 warnings). That fix made the re-emission genuinely work, which then raised the follow-up question the user asked directly: since GitHub has no supported way to show a non-blocking "passed with warnings" signal at the PR's checks-list/merge-button level (confirmed via docs research and the user's own screenshots — annotations only ever land on "Files changed" for lines in that PR's diff, or on the run's own Summary/Annotations page; a `neutral` conclusion can only be set by a separate custom Checks-API check run, not by a normal workflow job), the entire non-failing-annotation/job-summary apparatus no longer served its original goal (visibility near the merge button) and only added complexity for a benefit (a warnings table one click away) the user judged not worth keeping | Reversed the Phase 7.1 Guiding Principle: `dotnet build` now adds `-warnaserror` (fails the job on any Warning-severity diagnostic — confirmed via research that `EnforceCodeStyleInBuild=true` only reports IDE rules already configured at `warning`/`error` severity in `.editorconfig`, never `suggestion`/`silent`, so this doesn't silently expand scope); `dotnet format --verify-no-changes --severity info` no longer discards its exit code (`\|\| true` removed) — since it already fails on any severity ≥ `info`, i.e. everything, it fully covers the sub-Warning/suggestion-level gap `-warnaserror` can't reach. `.github/scripts/annotate-diagnostics.sh` was deleted entirely (redundant now that the .NET SDK's own native annotations plus a failing job/red-X already give full visibility, with no custom parsing needed). Deliberately accepted as a live test: ~20 pre-existing CA1822/CA1823 warnings in `DiscogsApiClient.cs` are intentionally left unfixed so the next CI run demonstrates the new policy actually blocks a merge, rather than fixing them first |
| 2026-08-21 | Phase 7.2 `publish-nuget.yml` passes `IncludeSymbols=true`/`SymbolPackageFormat=snupkg` as one-off `dotnet pack` properties instead of adding them to `DiscogsApiClient.csproj` | The csproj already has `GeneratePackageOnBuild=True`, so permanently adding those properties would make every local `dotnet build`/`dotnet pack` start emitting a `.snupkg` as a side effect; scoping them to the workflow's explicit pack invocation keeps local dev builds unchanged | `dotnet pack ... -p:IncludeSymbols=true -p:SymbolPackageFormat=snupkg`; validated locally that both `DiscogsApiClient.5.0.0.nupkg` and `.snupkg` are produced |
| 2026-08-21 | Package version resolved via `dotnet msbuild -getProperty:PackageVersion` rather than parsing the `.csproj` XML or duplicating the version elsewhere | `-getProperty` (MSBuild 17.8+/.NET 8 SDK+) reads the fully-evaluated MSBuild property directly, avoiding a brittle regex/XML parse of the csproj and staying correct even if the version is ever set via an import instead of an inline literal; confirmed working locally (printed `5.0.0` for the current csproj) | Also confirmed via Microsoft Learn docs that `-getProperty` is a supported CLI switch, not just a raw MSBuild flag that happens to pass through `dotnet msbuild` |
| 2026-08-21 | `dotnet nuget push "./nupkg/*.nupkg" --skip-duplicate` pushes both the `.nupkg` and its matching `.snupkg` in one command — no separate push step for the symbol package | Confirmed via the official `dotnet nuget push` Microsoft Learn docs: the `-n\|--no-symbols` option is described as "Doesn't push symbols (even if present)", i.e. symbol packages are pushed automatically by default whenever a matching one exists alongside the `.nupkg` being pushed | Simpler workflow — a single push step handles both artifacts; `--no-symbols` is never passed |
| 2026-08-21 | Phase 7.2 implemented on branch `phase7-2-nuget-publish-workflow` off `modernization`, keeping `NUGET_API_KEY`/`NUGET_TEST_API_KEY` repository-secret creation as an explicit open item rather than treating the phase as blocked on it | Creating repository secrets requires access to GitHub repo settings that isn't available to an agent; the workflow file, docs, and local build/pack validation are all completable independently of that step | `docs/MODERNIZATION_PLAN.md` §7.4 and the Phase 7 acceptance criteria explicitly track "configure both API key secrets" and "at least one successful end-to-end test-server publish validated" as the two remaining pending items, to be closed out by the user before Phase 8 |
| 2026-08-21 | Replaced `NUGET_API_KEY`/`NUGET_TEST_API_KEY` long-lived API key secrets with NuGet Trusted Publishing (OIDC via `NuGet/login@v1`) for both push targets | User flagged that nuget.org's own docs (https://learn.microsoft.com/en-us/nuget/nuget-org/trusted-publishing) discourage API keys in favor of Trusted Publishing; since neither API key secret had actually been created yet (still an open item from the prior entry), there was nothing to migrate away from — switching straight to OIDC was strictly simpler than implementing API keys first and migrating later. Confirmed via `NuGet/login`'s own README that it accepts optional `token-service-url`/`audience` inputs for non-default (e.g. test-server) NuGet-compatible endpoints, and via community reports (a `NuGet/login` GitHub issue) that `int.nugettest.org` supports trusted publishing with `token-service-url: https://int.nugettest.org/api/v2/token` / `audience: https://int.nugettest.org` | Added `id-token: write` to job permissions; two mutually-exclusive `NuGet/login@v1` steps (`login_prod` for nuget.org defaults, `login_test` overriding `token-service-url`/`audience` for int.nugettest.org), gated by the existing `publish_to_test_server` input; push step's `--api-key` reads `steps.login_prod.outputs.NUGET_API_KEY \|\| steps.login_test.outputs.NUGET_API_KEY`. Per user's explicit choices: a single `NUGET_USER` secret covers both targets (same nuget.org username assumed on both), and no GitHub Environment (e.g. `release`) was added, since the user opted to keep the workflow simple for now |
| 2026-08-21 | Removed the test-suite run and multi-SDK (`8.0.x`/`9.0.x`/`10.0.x`) setup from `publish-nuget.yml`, keeping only a single-SDK build → pack → push pipeline | User feedback: this workflow only publishes what already went through `ci-library.yml`'s full build+test+format+vulnerability gate on the PR that merged to `main` — re-running the test suite here duplicates a check that already passed, and installing 3 SDKs was only ever needed so each TFM's tests ran against a matching installed runtime, which no longer applies once tests aren't run here | Job renamed `Pack & Publish`; single `10.0.x` SDK (compiles net8.0/net9.0/net10.0 fine on its own since nothing executes test binaries); kept the explicit `dotnet build` step before `dotnet pack --no-build` since `GeneratePackageOnBuild=True` means `dotnet pack` alone does not build first (confirmed locally via `NU5026`) |
| 2026-08-21 | `publish-nuget.yml` was temporarily cherry-picked onto its own branch and merged to `main` ahead of the rest of Phase 7.2, purely to unblock end-to-end test-server validation | Discovered that GitHub only registers a `workflow_dispatch`-triggered workflow (making it dispatchable via UI/API/`gh`) once its file exists on the repository's **default branch** (`main`) — confirmed via `gh workflow list`/`gh api .../actions/workflows` both returning no result for `publish-nuget.yml` while it only existed on `phase7-2-nuget-publish-workflow`. The full Phase 7.2 branch (with docs/decision-log updates) wasn't ready to merge to `main` yet, so a minimal second branch (`ci-nuget-publish-workflow-only`) was created off `main`, containing only the workflow file via `git checkout phase7-2-nuget-publish-workflow -- .github/workflows/publish-nuget.yml`, PR'd and merged straight to `main` | Once merged, `gh workflow run publish-nuget.yml --ref phase7-2-nuget-publish-workflow -f publish_to_test_server=true` (dispatched via the Actions UI in practice) successfully ran the workflow *using the YAML from `phase7-2-nuget-publish-workflow`* (GitHub resolves `workflow_dispatch` definitions from wherever it's registered, but executes using the content at the specified `ref`) — run `32525525184` completed successfully: build → pack → OIDC login (test server) → pushed both `DiscogsApiClient.5.0.0.nupkg` and `.snupkg` to `int.nugettest.org` (confirmed via job logs: separate "Your package was pushed." for `/api/v2/package` and `/api/v2/symbolpackage`); package became visible on the test server's `v3-flatcontainer` index shortly after. This satisfies the Phase 7 acceptance criterion for an end-to-end test-server publish |
| 2026-08-21 | Removed explanatory inline `#` comments from `publish-nuget.yml`'s YAML | User asked for fewer comments in the workflow; step names plus the docs/CI_CD.md write-up already carry the same context, so inline comments were redundant | Comments removed from the `Pack library` and `Upload package artifact` steps; no functional change |
| 2026-08-21 | Confirmed the strict-fail policy live: `Build` passed cleanly (0 Warning-severity build diagnostics — the CA1822/CA1823 findings only ever surface via `dotnet format`, never `dotnet build`, even on a clean rebuild), `Format check` correctly failed both `ci-library.yml` and `ci-demo.yml` due to the pre-existing warnings, blocking PR #20 as intended | User feedback after seeing the real failing run: (1) `ci-library.yml`'s job name (`Build, Test & Analyze`) and `ci-demo.yml`'s (`Build & Analyze`) were too similar to tell apart from a run's job list, which shows job names rather than workflow names; (2) the `Test with coverage` step had no `if: always()`, so a Format-check failure (which leaves Build's binaries intact) still skipped the entire test run, which then cascaded into `dorny/test-reporter`/`ReportGenerator-GitHub-Action`/`cat SummaryGithub.md` all failing on missing TRX/Cobertura files, adding failure noise unrelated to the real problem | Renamed job display names to `Build, Test & Analyze (Library)` / `Build & Analyze (Demo)`; added `if: always()` to `Test with coverage` so it still runs whenever Build produced binaries (a genuine compiler-error build failure now just fails the test step fast with a clear "not built" error instead of masking it as a skip); gated `Publish test results` (`fail-on-empty: false`), `Generate coverage summary`, `Add coverage to job summary`, and `Upload coverage report artifact` behind `hashFiles(...)` checks on their actual required input files, so a genuine build failure with zero test output no longer cascades into unrelated report-generation failures |
| 2026-08-21 | Confirmed `global.json`'s `test.runner: Microsoft.Testing.Platform` setting cannot be replaced with a `dotnet test` CLI flag | User asked whether the MTP-runner selection could move from `global.json` to a command-line parameter to avoid the extra file; research confirmed the .NET 10 SDK only supports selecting the runner via `global.json` — there is no `dotnet test --runner`-style CLI flag (the pre-.NET-10 `-p:TestingPlatformDotnetTestSupport=true` MSBuild-property workaround doesn't apply once a .NET 10 SDK is involved, matching the reasoning already in this file for why `global.json` was added) | No change — `global.json` remains the only viable mechanism, decision confirmed rather than reversed |
| 2026-08-21 | Reintroduced custom parsing/annotation generation via `.github/scripts/format-check.sh` (superseding the "no custom parsing needed" reasoning in the earlier `annotate-diagnostics.sh` deletion entry) | User wanted the `Format check` job-summary table grouped by finding (Rule/Severity/Message/Locations) instead of a flat list, and later flagged real bugs in it (a Library-vs-Demo table-format mismatch, later root-caused to a comma-vs-`<br>`-separator dedup bug carried over from a removed `Projects` column) | `dotnet format`'s own native annotations are per-exact-occurrence and un-groupable; `format-check.sh` parses its plain-text output, groups findings by `(severity, rule, message)`, sorts by severity rank then rule, dedupes `<br>`-joined locations, and renders both `::error`/`::warning`/`::notice` annotations and an HTML job-summary table |
| 2026-08-21 | Root-caused all current `dotnet format` false positives (`IDE0060`, `IDE0005`) to `MSBuildWorkspace` never resolving the analyzer-only `ProjectReference` to `DiscogsApiClient.SourceGenerator`, so the source generator never runs during `dotnet format`'s analysis | `-v diagnostic` verbosity surfaced `"Found project reference without a matching metadata reference"` for the generator project; confirmed via CI log triage that only these 2 rules ever fire, across 13 unique files (`DiscogsApiClient.cs` for `IDE0060`; `DiscogsApiClient.cs`, `Contract`/`QueryParameters` files, and `DiscogsApiClient.Tests/Serialization/EnumConverterTests.cs` for `IDE0005`) | This is a structural, un-fixable-from-our-side `dotnet format` limitation (not a `.csproj` issue — real `dotnet build`/`CoreCompile` resolves the generator fine); informed the exclusion decision below |
| 2026-08-21 | `IDE0060` promoted to real `dotnet build` enforcement (`.editorconfig`: `warning` severity); `IDE0005` left at `.editorconfig` default (`suggestion`), enforced only via VS's own live analysis, neither promoted to build nor left in CI's `dotnet format` check | Empirically confirmed via a clean rebuild with `EnforceCodeStyleInBuild=true`: `IDE0060` produces 0 false positives on real build (generator-aware) while still catching a genuinely-unused parameter planted in a scratch test file; `IDE0005` cannot be enforced by build at all regardless of severity (MSBuild's own SDK targets require `GenerateDocumentationFile=true` to even run the analyzer — a separate, unrelated, long-standing Roslyn limitation, [dotnet/roslyn#41640](https://github.com/dotnet/roslyn/issues/41640)) — user explicitly ruled out enabling XML docs as a workaround. Pragma-suppressing `IDE0005` per-file was tested and confirmed to work but rejected: too many files (11+) would need annotating just to satisfy a CI check | `format-check.sh` excludes both `IDE0005 IDE0060` via `--exclude-diagnostics` (configurable through `FORMAT_CHECK_EXCLUDED_DIAGNOSTICS`); user separately enabled VS's "Background analysis scope" = "Entire Solution" locally so `IDE0005` is still reliably caught while editing, regardless of which file is open — VS's own live analysis resolves the generator correctly, unlike `dotnet format` |
| 2026-08-21 | Confirmed `dotnet format style`/`dotnet format analyzers` don't hide any other currently-lurking false positive, and that dropping `style`/`analyzers` scope from CI entirely (leaving only `whitespace`) was rejected | Isolated `dotnet format style --severity info` and `dotnet format analyzers --severity info` runs confirmed `style` only ever reports `IDE0005`/`IDE0060` (nothing else) and `analyzers` reports nothing at all currently; but `.editorconfig` has ~50 `csharp_style_*`/`dotnet_style_*` convention rules (primary constructors, expression bodies, collection expressions, etc.) configured at `suggestion`/`silent`, which `EnforceCodeStyleInBuild` never surfaces regardless of severity — only `dotnet format style` checks them | Kept the full `dotnet format` invocation (not narrowed to `whitespace`-only) with only `IDE0005`/`IDE0060` excluded, so every other style/analyzer rule stays actively CI-enforced |
| 2026-08-21 | Added `.github/` (workflows, scripts, `copilot-instructions.md`) under `/Solution Items/.github/` in `src/DiscogsApiClient.slnx` | User wanted the CI/CD files visible and editable from the IDE's Solution Explorer, mirroring the existing `docs/` folder pattern (repo-root files weren't otherwise represented in the solution) | Verified `dotnet build -t:Restore` still succeeds after the `.slnx` edit; not added to `demo/DiscogsApiClientDemo.slnx` per explicit user choice, since that solution has no existing Solution Items precedent |
| 2026-08-21 | Manually validated all 4 representative CI failure modes live against PR #20 (build warning, build error, format violation, failing test), each via a temporary probe commit that was pushed, confirmed, then reverted | User wanted end-to-end proof the pipeline actually blocks each failure category at the right step, not just a design review | All 4 confirmed correct: build warning/error both fail the `Build` step (`-warnaserror`); format violation fails only `Format check`; failing test fails `Test with coverage` + `Publish test results`. Noted (no fix needed) that MSBuild logs each build diagnostic multiple times — once per multi-targeted TFM plus the end-of-build summary — a stock `dotnet build` behavior already documented in AGENTS.md, not introduced by the CI scripts |
| 2026-08-21 | Refreshed `DiscogsApiClient.csproj`'s stale `PackageReleaseNotes` (previously still described a v4.1.1 bugfix) with a short v5.0.0 summary, after discovering NuGet renders it as **unformatted plain text** — a long single-paragraph first draft (with inline bullet dashes and embedded URLs) rendered as an unreadable wall of text on nuget.org's Release Notes tab, confirmed live via two throwaway pre-release pushes (`5.0.0-test1`/`5.0.0-test2`) to `int.nugettest.org` since NuGet packages are immutable and a real `5.0.0` push can't be overwritten/re-verified once published. Also researched whether `PackageReleaseNotes` is still a legitimate, commonly-used field: confirmed via Microsoft's own [Package authoring best practices](https://learn.microsoft.com/nuget/create-packages/package-authoring-best-practices) doc that it's still explicitly recommended (not deprecated), typically as a short summary + link to a fuller external changelog — matching the direction taken here | Final value: two short lines — "v5.0.0 is an extensive modernization/rewrite of this library, including breaking changes." followed by plain-text links to `docs/CHANGELOG.md` and `docs/MIGRATION_GUIDE.md` (each on its own line, since real newlines in the MSBuild property value are preserved verbatim in the packed `.nuspec` and rendered as line breaks by nuget.org). Verified correct rendering via screenshot before reverting `PackageVersion` back to the real `5.0.0`; the two throwaway pre-release versions remain permanently listed on `int.nugettest.org` (test server only, not production — harmless, and expected since packages can't be deleted, only unlisted) |
