...
public sealed class ChangeInventoryListingsAsByteArrayTests(DiscogsApiClientFixture fixture)
{
    ...
    {
        var exception = await Assert.That(async () => await _unauthenticatedApiClient.ChangeInventoryListings((byte[])null!, cancellationToken))
    ...
    {
        var csvContent = "listing_id,price\n1,10.00"u8.ToArray();
    ...
    {
        var csvContent = Encoding.UTF8.GetBytes("listing_id,price\n999999999,10.00");
