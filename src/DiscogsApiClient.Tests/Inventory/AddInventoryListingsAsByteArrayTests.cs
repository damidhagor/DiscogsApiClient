...
public sealed class AddInventoryListingsAsByteArrayTests(DiscogsApiClientFixture fixture)
{
    ...
    {
        var exception = await Assert.That(async () => await _unauthenticatedApiClient.AddInventoryListings((byte[])null!, cancellationToken))
    ...
    {
        var csvContent = "release_id,price,media_condition\n1,10.00,Near Mint (NM or M-)"u8.ToArray();
    ...
    {
        var csvContent = Array.Empty<byte>();
    ...
    {
        var csvContent = Encoding.UTF8.GetBytes("release_id,price,media_condition\n999999999,10.00,Near Mint (NM or M-)");
