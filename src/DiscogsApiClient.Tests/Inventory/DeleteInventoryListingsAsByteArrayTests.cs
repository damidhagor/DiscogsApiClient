...
public sealed class DeleteInventoryListingsAsByteArrayTests(DiscogsApiClientFixture fixture)
{
    ...
    {
        var exception = await Assert.That(async () => await _unauthenticatedApiClient.DeleteInventoryListings((byte[])null!, cancellationToken))
    ...
    public async Task DeleteInventoryListings_ShouldDeleteListing_WhenListingIdIsValid(CancellationToken cancellationToken)
    {
        ...
        try
        {
            var csvContent = Encoding.UTF8.GetBytes($"listing_id\n{createResponse.ListingId}");
